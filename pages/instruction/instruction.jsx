...
const Instruction = () => {
  ...
  const deleteAdmin = (id) => {
    if (id) {
      Swal.fire({
        title: "Are you sure?",
        text: "You want to delete instruction?",
        icon: "warning",
        ...
        if (result.value) {
          menuService.deleteinstruction(id?._id).then((res) => {
            getData();
            alertService.success(res.message, {
              keepAfterRouteChange: true,
            });
            Swal.fire("Delete!", "Your instruction is deleted.", "success");
          });
        } else if (result.dismiss === Swal.DismissReason.cancel) {
          Swal.fire("Cancelled", "Your instruction is safe", "error");
        }
      });
    }
  };
  ...
  const getData = async () => {
    showLoader();
    await menuService.getAllinstruction(localData?._id).then((response) => {
      hideLoader();
      response.payload.Instruction?.map((e, index) => {
        e.index = index + 1;
        return e;
      });
      setComments(response.payload.Instruction);
    });
  };
  ...
  return (
    <>
      <Head>
        <title>SIP Interview - Instruction</title>
        <meta
          name="description"
          content="Meta description for the Instruction Page"
        />
      </Head>
